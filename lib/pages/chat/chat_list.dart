...
class ChatListItem extends StatelessWidget {
  const ChatListItem({Key? key,required this.chat,required this.state}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return ListTile(
      onTap: (){
        ...
        state.notify();
        Go.pushSlideAnim(context, ChatBuilder(state: state,));
      },
      leading: SizedBox(
      ...
      title: Text(chat.personalInformations[1].personalInformation?.fullName??''),
      subtitle: chat.chatMessages.isEmpty?null:Text(chat.chatMessages.last.text??'',
      style: TextStyle(
        height: 2
      ),
      maxLines: 1,
