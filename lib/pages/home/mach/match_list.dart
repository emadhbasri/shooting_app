...
class _MatchListState extends State<MatchList> {
  late MatchState mainState;
  @override
  void initState() {
    super.initState();
    mainState = Provider.of(context,listen: false);
    mainState.getCountries();
  }
  @override
  Widget build(BuildContext context) {
    if(mainState.matchPage){
      return Match();
    }else{
      return Scaffold(
        // appBar: AppBar(
          ...
          children: [
            ...
            Consumer<MatchState>(
                builder: (context, state, child) =>
                    Column(
                      mainAxisSize: MainAxisSize.min,
                      children: state.leagues.map((e) =>
                          MatchListItem(league: e,state: state,),
                      ).toList(),
                    )
            ),
...
class MatchListItem extends StatefulWidget {
  const MatchListItem({Key? key,required this.league,required this.state}) : super(key: key);
...
class _MatchListItemState extends State<MatchListItem> with AutomaticKeepAliveClientMixin{
  bool isOpen=false;
  @override
  Widget build(BuildContext context) {
    super.build(context);
    return ExpansionTile(
      trailing: isOpen?Icon(Icons.keyboard_arrow_up_outlined):Icon(CupertinoIcons.chevron_forward),
      initiallyExpanded: isOpen,
        onExpansionChanged: (e){
        if(e && widget.league.matchs.isEmpty){
          widget.state.selectedLeagueIndex=widget.state.leagues.indexOf(widget.league);
          widget.state.notify();
      ...
      children: widget.league.matchs.map((e) =>
      Column(
        children: [
          MatchItem1(match: e,state: widget.state,),
...
class MatchItem1 extends StatelessWidget {
  const MatchItem1({Key? key,required this.match,required this.state}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return InkWell(
      borderRadius: BorderRadius.circular(10),
      onTap: (){
        state.selectedMatchIndex=state.leagues[state.selectedLeagueIndex].matchs.indexOf(match);
          ...
          children: [
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Expanded(child: Text(match.home.name,maxLines: 1,overflow: TextOverflow.ellipsis,))
                  ],
                ),
                Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    ...
                    SizedBox(width: doubleWidth(4)),
                    Expanded(child: Text(match.away.name,maxLines: 1,overflow: TextOverflow.ellipsis,))
                  ],
                ),
              ],
            )),
            Expanded(child: Column(
              children: [
                ...
                Text(match.homeGoals==null?'0':match.homeGoals.toString()),
                Spacer(),
                Text(match.awayGoals==null?'0':match.awayGoals.toString()),
                SizedBox(height: doubleHeight(0.7)),
              ],
            )),
            Expanded(child: Column(
              ...
              children: [
                ...
                Text(match.fixture.status,style: TextStyle(
                    fontSize: 13
                )),
                Spacer(),
if(match.fixture.elapsed!=null)
                Text('${match.fixture.elapsed} \' ',style: TextStyle(
                    fontSize: 13
                )),
                SizedBox(height: doubleHeight(0.7)),
              ],
